...
```
### Build with Docker (macOS / other hosts)
The Rockchip toolchain is Linux-only. On **macOS**, use Docker Desktop and `./docker-build.sh` instead of a native host install. The wrapper builds an Ubuntu 22.04 image, runs `./build.sh` inside the container, and on Darwin syncs sources into a **case-sensitive** Docker volume (APFS bind-mounts break the kernel tree where pairs like `ipt_ECN.h` / `ipt_ecn.h` collide).

```shell
# Once: build the image
./docker-build.sh --build-image

# Select board (interactive) — creates .BoardConfig.mk on the host / volume
./docker-build.sh lunch

# Full build (same as ./build.sh with no args)
./docker-build.sh

# Or pass any build.sh target
./docker-build.sh uboot
./docker-build.sh kernel
./docker-build.sh all
...
* The cross-compilation toolchain needs to be set Environment Variables (native Linux host; not required when using `./docker-build.sh`)
```
